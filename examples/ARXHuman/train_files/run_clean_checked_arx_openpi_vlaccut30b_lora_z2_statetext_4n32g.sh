...
export RUN_ID=${SLURM_JOB_ID}_arx_openpi_tube_human_v2_vlaccut30b_qwenpi_v3_lora_z2_statetext_pd1_4n32g_cleanchecked
...
srun --jobid "${SLURM_JOB_ID}" --ntasks="${SLURM_NNODES}" --ntasks-per-node=1 env -u SHELLOPTS bash -c '
  ...
  accelerate launch \
    ...
    --trainer.lora.module_path qwen_vl_interface.model \
    --trainer.lora.r 16 \
